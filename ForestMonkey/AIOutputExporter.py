...
def process_bydir(ori_img="", gt_mask="", predict_mask="", mask_contain_type=False, only_type=False, iou_threshold=0.05, iou_smaller_than=[], iou_larger_than=[], extend_on=None, mask_ignore_threshold=0.0005, fp=False, merged=False, separate=False):
    label = {}
    ...
    for i in tqdm(all_img, desc="Processing all outputs"):
        ...
        for g in gt_m:
            matched_pred = []
            result = {"detected": 0, "not-detected": 0, }
            if mask_contain_type:
                result = {"detected": 0, "not-detected": 0, "correct-type-classified": 0, "wrong-type-classified":0}
            used_pred = 0
            for p in pred_m:
            ...
            if len(matched_pred) > 0:
                result["detected"] = 1
                if separate:
                    if len(matched_pred) > 1:
                        result["separate-detected"] = 1
                    else:
                        result["separate-detected"] = 0
                if mask_contain_type:
            ...
            used_gt += 1
        if merged:
            for idx in range(len(label[i]["results"])):
                label[i]["results"][idx]["merged-detected"] = 0
        ...
        if fp:
            for idx in range(len(label[i]["results"])):
                label[i]["results"][idx]["false-positive"] = 0
            if len(left_pred_idx) >= 1:
                for p in left_pred_idx:
                    result = {"detected": 0, "not-detected": 1, "false-positive": 1}
                    if mask_contain_type:
                        result = {"detected": 0, "not-detected": 1, "false-positive": 1, "correct-type-classified": 0,
                                  "wrong-type-classified": 1}
                    if iou_smaller_than != []:
                        for iout in iou_smaller_than:
                            result["iou-less-" + str(iout)] = 1
    ...
    if mask_contain_type and only_type:
        for i in label:
            for j in range(len(label[i]["results"])):
                label[i]["results"][j] = {"correct-type-classified": label[i]["results"][j]["correct-type-classified"],
                                          "wrong-type-classified": label[i]["results"][j]["wrong-type-classified"]}
